...
BOOL CSystemDiagDlg::OnInitDialog()
{
	...
	else if (pDoc->nILFiles > 0) // if a specified group is given
	{
...
void CSystemDiagDlg::GetValues()
{
	...
	pDoc->GetInputLocations(m_InputList,m_nIList,values);
	/*
	for (int i = 0; i < m_nIList; i++)
	{
		...
		PeekMessage( &Msg,         // message information
			...
			PM_REMOVE ); //wRemoveMsg//| PM_NOYIELD
		...
		{
			wText = Strings.get(IDS_M_RETRIEVING_LOCATION);
			wText += " " + IntToString(m_InputList[i]);
			CollectProgress.SetInfo(100 * i / m_nIList, wText.GetBuffer(0));

			pDoc->StartCommBlock(10);
			pDoc->commObject->ExecuteICommand(m_InputList[i],d, NULL, 2);
			pDoc->EndCommBlock();
			values[i] = d;
		}
	}
