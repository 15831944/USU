								   ...
								   int wChamber) // = 0
{
	...
	if (!pDoc->GetInputLocations(&controlLocation,1,&oldValue))
	{
		return FALSE;
		// write to error log
	}
	...
	if (controlNumber)
	{
		SetInputLocation(controlLocation, controlNumber);
	}
	return TRUE;
}
...
BOOL CCommOperations::SetLocationAndVerify(int wLocation, double &oldValue, char *newValue, int nRepetitions)
{
	...
	for (int t = 0; t < nRepetitions; t++)
	{
		...
		{
			if (pDoc->GetInputLocations( &wLocation, 1, &oldValue ))
			{
...
//	DWORD totalTime;
	...
	int expectedSize;
	if (PingCR10(20) || PingCR10(50) || PingCR10(100) || PingCR10(100)){
		...
		} while (pause < 150 + (unsigned) InputLocRequests * 2);
	...
	else
	{
		do {
			...
			for (int i = preadBufPos; i < readBufPos; i++){
				...
				/*else if ((readBuf[i] == '>' || readBuf[i] == '<') && DoSwitchMode)
				{
...
BOOL CCommOperations::RetrieveFinalStorage(HANDLE FSFilesOut[], CLADAControlModuleDoc * pDoc, BOOL GetLastFSL)
{
	...
	if (!downloadCancelled && LocsToRetrieve > 0 && GetLastFSL)
		GetFinalStorage(value, FSFilesOut, pDoc, LocsToRetrieve);
	...
	BOOL DoInputLocations, int *InputLocList, int InputLocRequests)
{
	...
	if ( !IsRequestDifferent(DoInputLocations, InputLocList, InputLocRequests) 
			&& !DoPortFlags && !DoUserFlags && !ForceSetJCommand )
		return TRUE;
	
	DWORD sTime = timeGetTime();
	int expectedSize;

	if (PingCR10(20) || PingCR10(100) || PingCR10(500) || PingCR10(1000)){
		...
		//for (int t = 0; t < 5; t++){
			...
			} while (timeGetTime() - LastReadTime < 100 + (unsigned) i * 2);
...
const char * CCommOperations::GetCR10Time(CTime *returnTime)
{
	...
	//int location = 1;
	//double data;
	//pDoc->GetInputLocations(&location,1,&data);
	//return timeBuf;
	EndCommOperation();
	return NULL;
}
...
const char * CCommOperations::ProcessCR10Time(CLADAControlModuleDoc * pDoc)
{
	...
	if (pDoc)
	{
		...
		pDoc->CR10Time = CTime(curTime.GetYear(),curTime.GetMonth(),
								curTime.GetDay(),hours,minutes,seconds);
	...
	if (hours > 24 || minutes > 60 || seconds > 600)
	{
		...
		// means time is bad!!!!!!!
		pDoc->m_ErrorFiles->OpenFiles();
		pDoc->m_ErrorFiles->WriteHistory("Communicatin Failure","Bad time returned by CR10X");
		pDoc->m_ErrorFiles->CloseFiles();
	}
	return timeBuf;
}
		...
		for (int t = 0; t < 3; t++){
			...
			do {
				...
				if (readBufPos > 17)
				{
					EndCommOperation();
					pDoc->m_ErrorFiles->OpenFiles();
					pDoc->m_ErrorFiles->WriteHistory("Update Time","Successful");
					pDoc->m_ErrorFiles->CloseFiles();
					return TRUE;
				}
...
void CCommOperations::ToggleFlag(int wFlag, BOOL IsUser) // = TRUE
{
	memset(toggleFlags,0,sizeof(int) * 8 * 2);
	toggleFlags[wFlag-1][!IsUser] = 1;
	StartCommOperation(7);
	SetJCommand(IsUser,!IsUser,FALSE);
...
{
	if (GetFlag(wFlag,IsUser) != state)
		ToggleFlag(wFlag,IsUser);
	return TRUE;
}

BOOL CCommOperations::GetFlag(int wFlag, BOOL IsUser) // = TRUE
{
	StartCommOperation(8);
	SetJCommand(FALSE,FALSE,FALSE);	
	ExecuteKCommand(FALSE,0);
	EndCommOperation();
...
113: A_WRITE_20_MN	10			21
	...
	*/
	StartCommOperation(9);
	switch (freqSel)
	{
	case 0:  // every hour
		{
			if (SetLocationAndVerify(A_WRITE_1_HR, oldValue, 10) &&
				SetLocationAndVerify(A_WRITE_20_MN, oldValue, 21) &&
				SetLocationAndVerify(A_READ_20_MN, oldValue, 21) &&
	...
	case 1:  // every twenty minutes
		{
			if (SetLocationAndVerify(A_WRITE_20_MN, oldValue, 10) &&
				SetLocationAndVerify(A_WRITE_1_HR, oldValue, 61) &&
				SetLocationAndVerify(A_READ_20_MN, oldValue, 2) &&
...
BOOL CCommOperations::DetermineCR10ProgramType(ControlStateType &cState)
{
	...
	int nChambers = 1;
	//, SelectWetupSettings, StartWetup, WetupInProgress, NoControlState};
	for (int i = 0; i < 7; i++)
	{
		int location = IL_PROGRAM_TYPE;
		if(pDoc->GetInputLocations(&location, 1, &oldValue))
		{
			...
			//	continue;
			//}
			if (nChambers == 1 && round(oldValue) >= 32 && round(oldValue) < 64)
			{
				// 1 chamber regular
				cState = Regular;
			}
			else if (nChambers == 2 && round(oldValue) >= 64 && round(oldValue) < 96)
			{
				// 2 chamber regular
				cState = Regular;
			}
			else if (round(oldValue) >= 96 && round(oldValue) < 128)
			{
				// wetup
				cState = StartWetup;
			}
...
BOOL CCommOperations::LoadCalibrationFile(CString fName)
{
	...
	{
		ret=(SetInputLocation(location,newvalue) && ret);
...
BOOL CCommOperations::SetInputLocation(int wLocation, double newValue, BOOL verify, BOOL useDocIndex)
{
	return TRUE;
}


void CCommOperations::StartCommOperation(int wFunctionIsCalling)
{
	static int prevCall;
	...
	if (waitResult == WAIT_TIMEOUT)
	{
		...
		// write to error log
		char Message[80];
		wsprintf(Message,"Time Out Occured.  ThreadsInCommBlock = %i, PrevCall = %i, CurrentCall = %i",pDoc->ThreadsInCommBlock, prevCall, wFunctionIsCalling);
