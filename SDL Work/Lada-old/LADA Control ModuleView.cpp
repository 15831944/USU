	...
	case UPDATE_GRAPH_TIMER:
		{
			...
			{
				for (int i = 0; i < pDoc->GraphCollection[n].nItems; i++)
				{
					graphList[j++] = pDoc->GraphCollection[n].DataIndex[i];
	...
	case TAKE_PICTURE_2:
		{
			...
			if (!IsInUse)
			{
				...
				//pDoc->m_History->WriteHistory("Event","Picture taken.");
	...
	case UPDATE_CR10_TIME:
		{
			...
			//if (CTime::GetCurrentTime() - pDoc->CR10Time > CTimeSpan(0,0,0,5))
			//{
			//pDoc->StartCommBlock(1);
			//const char * timeText = pDoc->commObject->GetCR10Time(&pDoc->CR10Time);
			//pDoc->EndCommBlock();
			//GetDlgItem(IDC_CR10TIME)->SetWindowText(timeText);
			//pDoc->CR10TimeUpdated = CTime::GetCurrentTime();
			//}
	...
	case SelectWetupSettings:
		{
			...
			GetDlgItem(IDC_TEXT3)->SetWindowText(Strings.get(IDS_SELECT_WETUP_SETTINGS));
			GetDlgItem(IDC_CONNECT)->SetWindowText(Strings.get(IDS_SET_WETUP_PARAM));
			GetDlgItem(IDC_CONNECT)->EnableWindow(TRUE);
		} break;
	case StartWetup:
		{
...
void CLADAControlModuleView::SetFinalStorageTimer(lpfnTimerCallback lpfnTimer, CTime &currentTime)
{
	...
	if (pDoc->nFSFiles)
	{
		...
		if (pDoc->m_FSCollectionParam.m_LastAttemptedCollectionTime != 0){
			...
			if (CTimeSpan(0,0,0,timeTo/1000) > pDoc->m_FSCollectionParam.m_NewFileFreq)
				MessageBox("Final Storage timer set to a value greater than pDoc->m_FSCollectionParam.m_NewFileFreq","Error",MB_ICONSTOP);
#endif
		...
		else
		{
			...
			pDoc->m_FSCollectionParam.m_LastAttemptedCollectionTime = currentTime;
		}
...
void CLADAControlModuleView::SetILTimer(int wIL, lpfnTimerCallback lpfnTimer, CTime &currentTime)
{
	...
	if (curILData->m_Do)//pDoc->saveLocations && 
	{
			...
			// means that we can collect data inbetween when it is written to a file
		{
			...
			if (curILData->m_LastAttemptedCollectionTime != 0)
			{
	...
	if (CTimeSpan(0,0,0,timeTo/1000) > curILData->m_RetrFreq)
		MessageBox("FIRST_IL_TIMER set for a time longer than curILData->m_WriteDataFreq","Error",MB_ICONSTOP);
#endif
		...
		if (curILData->m_LastAttemptedCollectionTime == 0)
			curILData->m_LastAttemptedCollectionTime = currentTime;
	...
	if (CTimeSpan(0,0,0,timeTo/1000) > curILData->m_WriteDataFreq)
		MessageBox("FIRST_IL_SAVE_TIMER set for a time longer than curILData->m_WriteDataFreq","Error",MB_ICONSTOP);
#endif
		...
		else
		{
			...
			//curILData->m_LastSaveTime = currentTime;
		}
