...
void CILProperties::OnOK() 
{
	...
	UpdateData();
	if (m_GroupName == CString(""))
	{
		MessageBox("This group must have a name.","Error",MB_ICONINFORMATION);
		GetDlgItem(IDC_ILGROUPNAME)->SetFocus();
		return;
	}
	if (m_NIL == 0)
	{
		CString eng = Strings.get(IDS_M_INSERT_LOCATIONS, ENGLISH) + m_GroupName+CString("\".");
		CString russ = Strings.get(IDS_M_INSERT_LOCATIONS, RUSSIAN) + m_GroupName+CString("\".");
		Strings.set(eng,russ,IDS_TEMP);
		MessageBox(IDS_TEMP,IDS_ERROR,MB_ICONINFORMATION);
