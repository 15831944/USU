...
void CManualSample::OnTimer(UINT nIDEvent) 
{
	...
	if (nIDEvent==0)
	{
		...
		{	
			...
			pDoc->GetInputLocations( &locations[0], 3, &data[0] );
		...
		else if (step1 == 2)
		{
			...
			pDoc->GetInputLocations( &locations[6], 5, &data[6] );
			pDoc->GetInputLocations( &locations[16], 3, &data[16] );
		...
		{
			SetTimer(0,20000,NULL);
			pDoc->GetInputLocations( &locations[3], 3, &data[3] );
			...
			pDoc->GetInputLocations( &locations[14], 2, &data[14] );

			data[14]+=data[6];
			data[15]+=data[7];
			
			pDoc->GetInputLocations( &locations[22], 2, &data[22] );
		...
		if (step1==4)
		{
			pDoc->GetInputLocations( &locations[11], 3, &data[11] );
			data[11]+=data[3];
			data[12]+=data[4];
			data[13]+=data[5];
			pDoc->GetInputLocations( &locations[19], 3, &data[19] );
...
double CManualSample::Comm(int location)
{
	...
	//	ret = -99999.00;
	//pDoc->EndCommBlock();
	pDoc->GetInputLocations(&location,1,&ret);
	return ret;
}
...
void CManualSample::OnOK() 
{
	if (WillOutput)
	{
		for (int x=0;x<8;x++)
		{
			print->data[x+3][1]=ValueToString(data[x+16],2);
			print->data[x+11][1]=ValueToString(data[x],2);
		}		
	}
	CDialog::OnOK();
}
...
BOOL CManualSample::PreTranslateMessage(MSG* pMsg) 
{
	if (pMsg->message == WM_KEYDOWN && pMsg->wParam == VK_F2)
	{
		...
		for (int y=0;y<3;y++)
		{
			...
			CString a=Strings.get(IDS_H5+y);
...
void CManualSample::OnStart1() 
{
	...
	FinishTime=CTime::GetCurrentTime() + CTimeSpan(0,0,0,65);  // 80 seconds
	int y;
	y=SetTimer(0,5000,NULL);
...
void CManualSample::GetLastResults()
{
	int locations[] = { ITemp1, ITemp2, ITemp3, ITemp4, ITemp5, ITemp6, ITemp7, ITemp8,
				   IDTemp1, IDTemp2, IDTemp3, IDTemp4, IDTemp5, IDTemp6, IDTemp7, IDTemp8,
				   ISat1, ISat2, ISat3, ISat4, ISat5, ISat6, ISat7, ISat8 };
	pDoc->GetInputLocations(locations,24,data);
