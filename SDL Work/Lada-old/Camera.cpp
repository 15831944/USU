...
//	pDoc->dataFolder=
	...
	delete [] m_PtrFileInfo;
	m_PtrFileInfo = new CString[1000];
	m_nFiles = 0;
...
//	CString look=now.Format("%B");
	...
	strcpy(temp,pDoc->dataFolder.GetBuffer(0));
	strcat(temp,tempf.GetBuffer(0));
	ofstream fout(temp);
	fout.close();
...
//	CreateStatusWindow();
//	m_ptrStatus->SetMessage(IDS_M_TAKING_PICTURE);
	...
	DWORD waitResult = WaitForSingleObject( cameraSuccessEvent, 15000 );
	ResetEvent( cameraSuccessEvent ); // sets event to non-signaled state.
