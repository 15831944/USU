...
{
	int locations[] = { m_MoistureLocation, m_LightOffLocation, m_LightOnLocation };
	double returnData[3];
	int loff, lon;

	pDoc->GetInputLocations(locations,3,returnData);
...
BOOL CParamDlg::UpdateCR10Parameters()
{
	...
	{
		BOOL SetSuccess = pDoc->SetControlParameters(m_wChamber, m_Moisture, m_24Hour, m_LightOn, m_LightOff, m_wSMPFreq );
		
		if (SetSuccess)
		{
			GetCR10Parameters();
			// important values
			//curControlParameters->m_MoistureLevel = m_Moisture;
			//m_Fan = curControlParameters->m_FanOnTime;
			//curControlParameters->m_24Hour = m_24Hour;
			//curControlParameters->m_LightOnTime = m_LightOn;
			//curControlParameters->m_LightOffTime = m_LightOff;
			//curControlParameters->m_SMPFreq = m_wSMPFreq;
			// THOSE VALUES ARE BEING SET BY THE CALL TO SETCONTROLPARAMETERS...
		}
		else
		{
