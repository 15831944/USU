...
BOOL CCameradialogDlg::OnInitDialog()
{
	...
	m_Frequency.AddString(Strings.get(IDS_F11));
	m_Frequency.AddString(Strings.get(IDS_F12));
	m_Frequency.AddString(Strings.get(IDS_F13));
	m_Frequency.AddString(Strings.get(IDS_F14));
...
void CCameradialogDlg::LoadPicture(int a)
{
	...
	else
	{
		...
		LoadBMPImage((LPCTSTR) command,*Bitmap,Palette);
		m_rpicture.SetBitmap((HBITMAP)*Bitmap);
	}
...
BOOL CCameradialogDlg::PreTranslateMessage(MSG* pMsg) 
{
	if (pMsg->message == WM_KEYDOWN && pMsg->wParam == VK_F2)
	{
			...
			{
				x->mask=LVCF_TEXT;
				m_FileBox.GetColumn(y,x);
				CString a=Strings.get(IDS_F7+y);
				LPTSTR temp=a.GetBuffer(0);
				x->pszText=temp;
				m_FileBox.SetColumn(y,x);
			}
		m_Frequency.ResetContent();
		m_Frequency.AddString(Strings.get(IDS_F11));
		m_Frequency.AddString(Strings.get(IDS_F12));
		m_Frequency.AddString(Strings.get(IDS_F13));
		m_Frequency.AddString(Strings.get(IDS_F14));
