...
class Array {
	public:
		...
		void InsertAddress(ArrayType &ptrObject, int Subscript);
...
template <class ArrayType>
void Array<ArrayType>::InsertAddress(ArrayType &iObject, int Subscript){
	// this function may only be used if this array
	// is an array of pointers.
	(*this)(length + 1);
	for (int i = length - 1; i > Subscript; i--)
		pPtrs[i] = pPtrs[i-1];
	pPtrs[i] = &iObject; // shallow copy
}
