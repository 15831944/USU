...
BOOL CLADASetupDlg::OnInitDialog()
{
	...
	{
		SetWindowText(Strings.get(IDS_PUMP1));
		m_numsteps=1;
		m_step=new Stepinfo[m_numsteps];
		m_StepsPerPage=new int [m_numsteps];
		m_StepsPerPage[0]=4;
		for (int x=0;x<m_numsteps;x++)
		{
			CString temp[9];
			for (int y=0;y<9;y++)
				temp[y]=Strings.get(IDS_PUMP1_STEP_1+10*x+y);
			m_step[x]=Stepinfo(x+1,Strings.get(IDS_PUMP1_STEP_0+10*x),m_StepsPerPage[x],temp,Strings.get(IDS_PUMP1+10*x));

		}
	}
		else if (Type==Pump2)
	{
		SetWindowText(Strings.get(IDS_PUMP2));
		m_numsteps=1;
		m_step=new Stepinfo[m_numsteps];
		m_StepsPerPage=new int [m_numsteps];
		m_StepsPerPage[0]=5;
		for (int x=0;x<m_numsteps;x++)
		{
			CString temp[9];
			for (int y=0;y<9;y++)
				temp[y]=Strings.get(IDS_PUMP2_STEP_1+10*x+y);
			m_step[x]=Stepinfo(x+1,Strings.get(IDS_PUMP2_STEP_0+10*x),m_StepsPerPage[x],temp,Strings.get(IDS_PUMP2+10*x));
...
void CLADASetupDlg::CheckCheck(int temp)
{
	...
	else
	{
		...
		{
			GetDlgItem(IDC_TEXT1)->SetWindowText(Strings.get(IDS_PUMP1_STEP_EXPLANATION_1+9*m_stepnumber+checknumber+CheckTrue));
			CString name=workingDirectory+"\\LadaPics\\Pump1"+char('0'+m_stepnumber)+char('0'+checknumber+CheckTrue)+".bmp";
   			LoadBMPImage(name.GetBuffer(0),*m_Bitmap,ppal);
		}
		else if (Type==Pump2)
		{
			GetDlgItem(IDC_TEXT1)->SetWindowText(Strings.get(IDS_PUMP2_STEP_EXPLANATION_1+9*m_stepnumber+checknumber+CheckTrue));
			CString name=workingDirectory+"\\LadaPics\\Pump2"+char('0'+m_stepnumber)+char('0'+checknumber+CheckTrue)+".bmp";
   			LoadBMPImage(name.GetBuffer(0),*m_Bitmap,ppal);
		}
...
//			GetDlgItem(IDC_TEXT1)->SetWindowText(Strings.get(IDS_AB0+10*m_stepnumber+checknumber));
	...
	{
 		int number=checknumber+9*m_stepnumber;
		if (number == 7 ||
			number == 9 ||
			number == 11 ||
			number == 13)
		{
			...
			else
			{
				...
				do
				{
					...
					percentComplete = (int)(100 * returnData[0] / returnData[1]);
					PumpProgress.SetInfo(percentComplete);
					
					past50 = (past50 || percentComplete > 50);
				...
				} while ((returnData[0]+0.1) < returnData[1] && !Cancelled && !IsFinished);
				PumpProgress.Close();
				if (Cancelled)
...
void CLADASetupDlg::ResetStrings()
{
	...
	else if (Type==Pump1)
	{
			...
			for (int x=0;x<m_numsteps;x++)
			{
				...
				m_step[x]=Stepinfo(x+1,Strings.get(IDS_PUMP1_STEP_0+10*x),m_StepsPerPage[x],temp,Strings.get(IDS_PUMP1_STEP_0+10*x));
			}
	}
	else if (Type==Pump2)
	{
			...
			for (int x=0;x<m_numsteps;x++)
			{
				...
				m_step[x]=Stepinfo(x+1,Strings.get(IDS_PUMP2_STEP_0+10*x),m_StepsPerPage[x],temp,Strings.get(IDS_PUMP2_STEP_0+10*x));
			}
	}
