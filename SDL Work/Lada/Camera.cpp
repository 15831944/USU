...
UINT SaveThread(LPVOID data)
{
	...
	if (waitResult==WAIT_TIMEOUT)
		MessageBox("WaitForSingleObject timed out while waiting for cameraSuccessEvent.");
	else
		tdlg->OnSave();
	return 0;
}
	...
	: CDialog(CCamera::IDD, pParent)
{
	...
	/* This is for the temprorary camera */
	...
	if (hLibrary != NULL)    
	{
		...
		{             
			MessageBox( "Error loading camera driver.  GetProcAddress(hLibrary, \"capCreateCaptureWindowA\") failed."/*"System file not present (AVICAP32.DLL).", 
...
BOOL CCamera::OnInitDialog() 
{
	...
	/*This is for the temporary camera*/
	...
	{
		wsprintf(message,"Error loading camera:\nNo driver was found (%i)",0);
...
//	BOOL capCaptureSingleFrameOpen(VCWin1);
		...
		{
			MessageBox( "capGrabFrame( VCWin1 ) failed."/*"No camera present.", 
			"Initialization Error", 
			 MB_ICONEXCLAMATION*/ );
		}
...
//	CString look=now.Format("%B");
	...
	if (LastCamera==1)
	{
		if (ctemp==".dib")
			return;
		BOOL temp=capFileSaveDIB(VCWin1,ctemp.GetBuffer(0));
	}
	else
	{
		if (m_Dib.width==0)
