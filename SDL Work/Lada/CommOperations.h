...
enum ControlStateType {Regular, StartWetup, WetupInProgress, NoControlState};
...
public:
	BOOL HasCommBeenClosed;
	BOOL StartCommOperation(int wFunctionIsCalling = 0);
	void EndCommOperation();
	BOOL SetInputLocation( int wLocation, double newValue, int wCall = 0, BOOL verify = FALSE, BOOL useDocIndex = FALSE);
	...
	BOOL ToggleFlag(int wFlag, BOOL IsUser = TRUE);
	BOOL GetFlag(int wFlag, BOOL & FlagState, BOOL IsUser = TRUE);
