...
void CInputLocationsPage::ResetGroupTimer()
{
	if (pDoc->InCommMode && pDoc->m_pMainView)
	{
		...
		// these three lines of code should cause the next timer to be set
		// to an offset of the frequency from the current time
		pDoc->m_ILData[m_wGroup].m_NextCollectionTime = 0;
		if (!pDoc->m_ILData[m_wGroup].m_SaveByAverage)
			pDoc->m_ILData[m_wGroup].m_LastSaveTime = 0;
