...
BOOL CCr10FunctionsDlg::OnInitDialog() 
{
	...
	if (pDoc->commObject)
	{
		int location = 1;
		double rData;
		pDoc->GetInputLocations(&location,1,&rData,35);
		//pDoc->CR10Time should now be updated
		m_CR10Time = pDoc->GetCR10Time().Format("%H:%M:%S");
		m_FIVATime = CTime::GetCurrentTime().Format("%H:%M:%S");
...
void CCr10FunctionsDlg::OnUploadprogram() 
{
	...
	if (UploadSuccess)
	{
		...
		{
			if (pDoc->SetControlParameters(i, pDoc->m_ControlParameters[i].m_MoistureLevel, pDoc->m_ControlParameters[0].m_MaxDosage, pDoc->m_ControlParameters[i].m_24Hour, pDoc->m_ControlParameters[i].m_LightOnTime, pDoc->m_ControlParameters[i].m_LightOffTime, pDoc->m_ControlParameters[i].m_SMPFreq ))
...
void CCr10FunctionsDlg::OnTimer(UINT nIDEvent) 
{
...
void CCr10FunctionsDlg::OnClose() 
{
	// TODO: Add your message handler code here and/or call default
	KillTimer(0);
	CDialog::OnClose();
