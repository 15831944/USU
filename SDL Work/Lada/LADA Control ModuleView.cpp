		...
		case Regular:
			{
				...
				{
					if (pDoc->Connect(TRUE, Regular))
					{
						if (pDoc->m_ControlMode == ManualControlMode) 
						{
							if (MessageBox("LSS is running in manual mode currently.  In manual mode all data will be collected, but automatic control features (pumps, lights, fans) will not work unless you switch to automatic control mode.  Would you like to switch to automatic control mode now?","Lada Software System",MB_ICONQUESTION|MB_YESNO)==IDYES)
							{
								BOOL setModeSuccess = pDoc->commObject->SetControlMode( AutomaticControlMode );
								if (setModeSuccess)
								{
									pDoc->m_ControlMode = AutomaticControlMode;
									SetControlModeWindow( AutomaticControlMode );
								}
...
BOOL CLADAControlModuleView::GetFinalStorage(BOOL ISWETUP)
{
	...
	CString s = theTime.Format( "%y-%m-%d %H;%M"); /// THIS IS WHERE YOU FORMAT THE TIME IN THE FILE NAME
	...
	{
		if (ISWETUP)
			wsprintf(fileNameBuff,"%s\\%s%s %s.%s", pDoc->dataFolder, "w",pDoc->m_FSOutputTable[wFile].m_ArchiveName, s, FinalStorageExtension);
		else
		...
		{
			if (!ISWETUP || (i!=4 && i!=34 && i!=35 && i!=48 && i!=52))
			{
				if (i)
					fHeader += '\t';
	...
	Success = pDoc->commObject->RetrieveFinalStorage(FSFilesOut, pDoc, TRUE);
