...
HANDLE CCommFunctions::hCommLog = NULL;
...
BOOL CCommFunctions::OpenConnection()
{
	...
	DataReceived = CreateEvent(NULL,TRUE,FALSE,"CR10_DATA_RECEIVED");
	return ( fRetVal ) ;
...
BOOL CCommFunctions::WriteCommBlock( const LPSTR lpData , DWORD dwBytesToWrite)
{
	...
	WriteToCommLog('T', (unsigned char *) lpData, dwBytesSent);
	return ( TRUE ) ;
		...
		if ((dwEvtMask & EV_RXCHAR) == EV_RXCHAR)
		{
			do {
				if (nLength = ReadCommBlock( TalkInfo, (LPSTR) abIn, MAXBLOCK ))
				{
					...
					WriteToCommLog('R',(unsigned char *)abIn,nLength);
					SetEvent(DataReceived);
				}		
			} while ( nLength > 0 ) ;
		}
	}
...
void CCommFunctions::OpenCommLog(char * folder)
{
	//CTime theTime;
	...
	hCommLog = NULL;
	//wsprintf(fileNameBuff,"%sComm Log %s.txt", folder, theTime.Format("%b %d %H;%M"));
	
	//hCommLog = CreateFile(fileNameBuff, 
	//	GENERIC_WRITE, 
	//	FILE_SHARE_READ, 
	//	NULL, 
	//	CREATE_ALWAYS, 
	...
	//if (hCommLog == INVALID_HANDLE_VALUE)
	//{
...
void CCommFunctions::CloseCommLog()
{
	if (hCommLog)
		CloseHandle(hCommLog);
}

void CCommFunctions::WriteToCommLog(char t, unsigned char *data, int length)
{
	timeb timebuf;
	ftime(&timebuf);
	char line[200];

	for ( int cpos = 0; cpos < length; cpos += 16)
	{
		if (cpos == 0)
		{
			wsprintf(line,"%.8s.%-03.3hu %c %3i bytes ", CTime(timebuf.time).Format("%H:%M:%S"), timebuf.millitm, t, length);
		}
		else
		{
			for (int i = 0; i < 25; i++)
		...
		for(int pos = 0; pos < 16 && pos + cpos < length; pos++)
		{
			wsprintf(&line[pos * 4 + 42], "%4u", data[pos+cpos]);
		}
		line[pos * 4 + 42] = '\n';
		line[pos * 4 + 43] = '\0';
		//DWORD bytesWritten;
		//WriteFile(hCommLog, line, strlen(line), &bytesWritten, NULL);
		OutputDebugString(line);
	}
}

void CCommFunctions::WriteToCommLog(char * message)
{
	timeb timebuf;
	ftime(&timebuf);
	char line[200];

	wsprintf(line,"%.8s.%-03.3hu %s\n", CTime(timebuf.time).Format("%H:%M:%S"), timebuf.millitm, message);
