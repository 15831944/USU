...
BEGIN_MESSAGE_MAP(CGraphButtonForm, CFormView)
	...
	ON_BN_CLICKED(IDC_GRAPH_SIZE, OnGraphSize)
	//}}AFX_MSG_MAP
...
{
	graphButton.AutoLoad(IDC_GRAPH_SIZE,this);
	graphButton.ModifyStyle(0, BS_AUTOCHECKBOX );
	graphButton.SetState(0);
...
void CGraphButtonForm::OnUpdate(CView* pSender, LPARAM lHint, CObject* pHint) 
{
	...
	{
		int wCollection = pDoc->PaneToCollectionIndex[whichWindow];

		for (int i = 0; i < numControls; i++)
		{
			if (i < pDoc->GraphCollection[wCollection].nItems )	
				numberBoxes[i]->SetWindowText(ValueToString(pDoc->newData[pDoc->GraphCollection[wCollection].DataIndex[i]]));
		}
	}
...
void CGraphButtonForm::OnGraphSize() 
{
	// TODO: Add your control notification handler code here
	CRect rect;
	((CWnd*)pDoc->m_ptrMainFrame)->GetClientRect(rect);
	int windowHeight = rect.Height();
	
	
	if (IsPaneMaximized)
	{
		pDoc->m_ptrMainFrame->m_wndToolBar.GetWindowRect(rect);
		windowHeight -= rect.Height();
		pDoc->m_ptrMainFrame->m_wndStatusBar.GetWindowRect(rect);
		windowHeight -= rect.Height();
		for (int wWindow = 0; wWindow < pDoc->nPanes; wWindow++)
		{
			pDoc->m_ptrMainFrame->m_wndSplitter2->SetRowInfo( wWindow, windowHeight / 3, 10 );
		}
		pDoc->m_ptrMainFrame->m_wndSplitter2->RecalcLayout( );
		IsPaneMaximized = FALSE;
	}
	else
	{
		for (int wWindow = 0; wWindow < pDoc->nPanes; wWindow++)
		{
			if (wWindow != whichWindow)
				//pDoc->m_ptrMainFrame->m_wndSplitter2->SetRowInfo( whichWindow, rect.Height(), 50 );
			//else
				pDoc->m_ptrMainFrame->m_wndSplitter2->SetRowInfo( wWindow, 0, 10 );
		}
		pDoc->m_ptrMainFrame->m_wndSplitter2->SetRowInfo( whichWindow, windowHeight, 10 );
		pDoc->m_ptrMainFrame->m_wndSplitter2->RecalcLayout( );
		IsPaneMaximized = TRUE;
	}
