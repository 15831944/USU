...
void WetupDialog::OnCancel() 
{
	...
	if (MessageBox(IDS_M_DOWNLOAD_FINAL,IDS_M_DOWNLOAD_FINAL,MB_YESNO)==IDYES)
	{
		KillTimer(1);
		do
		{
			success=pDoc->m_pMainView->GetFinalStorage(TRUE);	
		}
		while(!success && MessageBox(IDS_M_FINAL_FAILED,IDS_M_FINAL_FAILED,MB_YESNO)==IDYES);
	...
	else if(MessageBox(IDS_M_SURE_CANCEL, IDS_PROGRAM_NAME, MB_YESNO|MB_ICONQUESTION) == IDYES)
	{
		OnClose();
	}
...
BOOL WetupDialog::PrepareCR10()
{
	...
	*/
	...
	wSettings.DoModal();
	if (MessageBox(IDS_M_WETUP_READY,IDS_M_WETUP_READY,MB_YESNO)==IDNO)
	{
		return FALSE;
	}
	...
	int locationsToSet[] = {IL_TOTAL_TO_PUMP, IL_BURST_VOLUME, IL_N_ELAPSED_INTERVALS, IL_N_INTERVALS};

	double newValues[] = {pDoc->m_WetupInfo.m_VolumeToPump - pDoc->m_WetupInfo.m_VolumePumped, 
						  pDoc->m_WetupInfo.m_BurstVolume,
						  0,
						  nIntervals};

	for (int wIL = 0; wIL < 4; wIL++)
	{
		for (int wTry = 0; wTry < 3 && !pDoc->commObject->SetInputLocation(locationsToSet[wIL],newValues[wIL] , 109, TRUE); wTry++)
		{
			Sleep(50);
		}
		if (wTry == 3)
		{
			MessageBox("Failed to set wetup location in CR10X.","Communication Failure",MB_ICONWARNING);
			return FALSE;
		}
	}
