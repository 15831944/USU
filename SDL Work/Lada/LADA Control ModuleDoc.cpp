...
CLADAControlModuleDoc::CLADAControlModuleDoc()
{
	DataError = FALSE; 
	m_nTimeRequests = 0;
	nInGetInputLocations = 0;
	hDataRetrievalThread = NULL;
	DataRequest = NULL;
	CommWakeUpEvent = NULL;
...
BOOL CLADAControlModuleDoc::OnNewDocument()
{
	...
	if (nCameras > 0)
	{
		for (int x=0;x<=nCameras;x++)
		{
			m_CamerasEnabled=x+1;
			m_CameraData[x].m_Do=TRUE;
		}
		LoadCameras();
	}
...
void CLADAControlModuleDoc::Serialize(CArchive& ar)
{
	if (ar.IsStoring())
	{
		BOOL SaveDuringWetup = LoadBackGraphParameters;
		...
		for (int i = 0; i < nInputLocations; i++)
			ar.WriteString(InputLocationUnits[i]+CString("\n"));
		...
		ar.Write(&m_WetupInProgress,iSize);
	...
	else
	{
		...
		InputLocationUnits = new CString[nInputLocations];
		iSize = nInputLocations * sizeof(int);
		...
		for (int i = 0; i < nInputLocations; i++)
			ar.ReadString(InputLocationUnits[i]);
		...
		ar.Read(&m_WetupInProgress,iSize);
		...
		case ID_CHECK_OUT:
			{
				...
				CLADASetupDlg dlg(CheckOut,this);
				if (dlg.DoModal()==IDOK)
				{				
...
BOOL CLADAControlModuleDoc::Connect(BOOL StartTimers, ControlStateType desiredState, BOOL DoInitialization, BOOL FailOnWrongProgram )
{
	...
	if (m_pMainView)
	{
		m_pMainView->SetGraphTimer();
		m_pMainView->SetTimer(UPDATE_CR10_TIME, 5000, NULL);
	}
		...
		if (actualState != desiredState)
		{
			...
			if (MessageBox(message,Strings.get(IDS_PROGRAM_NAME),MB_ICONQUESTION|MB_YESNO)==IDYES)
			{
				...
				else
				{
					...
					{
						if (SetControlParameters(i, m_ControlParameters[i].m_MoistureLevel, m_ControlParameters[0].m_MaxDosage, m_ControlParameters[i].m_24Hour, m_ControlParameters[i].m_LightOnTime, m_ControlParameters[i].m_LightOffTime, m_ControlParameters[i].m_SMPFreq ))
...
BOOL CLADAControlModuleDoc::SetControlParameters(int whichChamber, double MoistLevel, int MaxDosage, BOOL In24Hour, int LightOn, int LightOff, int SMPFreq )
{
	...
	int locationsToSet[] = {m_MoistureLocation,m_LightOnLocation,m_LightOffLocation,IL_MAX_DOSAGE};
	double newValues[] = {MoistLevel,LightOn,LightOff,MaxDosage};

	for (int wIL = 0; wIL < 4; wIL++)
	{
		for (int wTry = 0; wTry < 3 && !commObject->SetInputLocation(locationsToSet[wIL],newValues[wIL] , 108, TRUE); wTry++);
		if (wTry == 3)
		{
			MessageBox("Failed to set control parameters in CR10X.","Communication Failure",MB_ICONWARNING);
			return FALSE;
		}
	}
	
	m_ControlParameters[whichChamber].m_MaxDosage = MaxDosage;
	...
	if (commObject->SetSMPTestFrequency(SMPFreq,whichChamber))
...
BOOL CLADAControlModuleDoc::GetInputLocations(int *ptrFirstLocation, int nLocations, double *ptrReturnData, int wCall, BOOL useDocIndex)
{
	static int prevCall = -1;
	

	if (nInGetInputLocations)
	{
		//MessageBox("Error- thread already in GetInputLocations",NULL,MB_ICONSTOP);
		char message[100];
		wsprintf(message,"Thread already in GetInputLocations.  curCall = %i, prevCall = %i", wCall, prevCall);
		m_ErrorFiles->WriteHistory("Error",message);
	}
	prevCall = wCall;
	...
	{
		m_ErrorFiles->WriteHistory("Error","Get input location failed.  commObject == NULL or InCommMode == NULL or DataRequest == NULL");
	...
	if (m_IsCommPaused)
	{
		m_ErrorFiles->WriteHistory("Notice","GetInputLocations called while m_IsCommPaused == TRUE");
	}
	
	// START CRITICAL SECTION...
	EnterCriticalSection(&CommCriticalSection);
	
		nInGetInputLocations++;
	...
	do 
	{
		...
		if (countera>150)
			Sleep(0);
		WaitResult = WaitForSingleObject( DataCompletion, 20 );
		RetrievalFailed = DataError;
		...
		{
			counterb++;
			if (Msg.message != WM_TIMER && (Msg.message < WM_MOUSEFIRST || Msg.message > WM_MOUSELAST) && (Msg.message < WM_KEYFIRST || Msg.message > WM_KEYLAST))
				DispatchMessage( &Msg );  // message information);
			else
			{
				// we do not want to process any WM_TIMER messages now...
				TimerMessageQueue.insertAtFront(Msg);
			}
		}
	...
	while ((WaitResult == WAIT_TIMEOUT)  && (countera < 200));

	ResetEvent( DataCompletion );
	nInGetInputLocations--;
	m_nActiveRequests--;
...
*/
	if (countera >=200)
	{
		m_ErrorFiles->WriteHistory("GetInputLocations failed","CounterA surpassed 200" );
	}

	while (TimerMessageQueue.removeFromBack(Msg))
		PostMessage(NULL,Msg.message,Msg.wParam,Msg.lParam);

	if (WaitResult == WAIT_TIMEOUT || RetrievalFailed)
	{
		if (RetrievalFailed)
		{
			m_ErrorFiles->WriteHistory("GetInputLocations failed", "DataError == TRUE");
...
void CLADAControlModuleDoc::EndDataRetrievalMode()
{
	...
	for (int i = 0; ThreadsInCommWaitTimer && i < 200; i++)
	{
		Sleep(10);
	}
	if (i == 200 && ThreadsInCommWaitTimer)
	{
		MessageBox("Could not exit, ThreadsInCommWaitTimer is non-zero");
		// write to error log... thread for collection would not exit...		
	}
	return;
}

void CLADAControlModuleDoc::FlushAllRequests()
{
	CInputRequest InputRequest;
...
CTime CLADAControlModuleDoc::GetCR10Time(BOOL ForceUpdate, int UpdateInterval)
{
	CTime curTime = CTime::GetCurrentTime();
	BOOL NeedUpdate = (curTime - CR10TimeUpdated).GetTotalSeconds() > UpdateInterval;
	if ((ForceUpdate || NeedUpdate) && commObject)
	{
		int wLocation = 1;
		double rData;
		GetInputLocations(&wLocation,1,&rData,38);
		return CR10Time;
	}

	return CR10Time + (curTime - CR10TimeUpdated);
}
