...
bool ReadObject(CString fileName, Object3d &object)
{
	...
	while (fin >> type)
	{
		...
		else if (type == 'f')
		{
			...
			{
				int v = numbers[i];
				object.neighboringFaces[v][0]++;
				object.neighboringFaces[v][object.neighboringFaces[v][0]] = wShape;
			}
			...
			object.shapes[wShape].NormalVector.SetVector(b1 * c2 - c1 * b2, c1 * a2 - a1 * c2, a1 * b2 - b1 * a2);


			// normalize this vector
			object.shapes[wShape].NormalVector.Normalize();
	...
	if (object.shadingStyle == Phong)
	{
		for (int i = 0; i < object.nVertices; i++)
		{
			object.verticeVector[i].SetVector(0,0,0);
			for (int j = 1; j <= object.neighboringFaces[i][0]; j++)
			{
				object.verticeVector[i] += object.shapes[object.neighboringFaces[i][j]].NormalVector;
			}
			object.verticeVector[i] /= object.neighboringFaces[i][0];

		}
	}
	//double dotProduct(CVector3d & Vector1, CVector3d & Vector2)
	//{
	//return (double)Vector1.x * Vector2.x + Vector1.y * Vector2.y + Vector1.z * Vector2.z;
	//}
	...
	//	for (int j = 1; j < object.neighboringFaces[i][0]; j++)
	//	{
	//		if (dotProduct(object.shapes[object.neighboringFaces[i][j]].NormalVector,
	//					   object.shapes[object.neighboringFaces[i][j+1]].NormalVector) < 0)
	//			flipCount++;
	//
	//	}
	//
