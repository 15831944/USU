...
BOOL ComChannel::SendEvent(Event &event, DString& server_ip, UINT server_port)
{
	...
	unsigned short lng = event.getPackLng();
	char* buffer = new char[lng];
	event.save(buffer, lng);
	int count = SendTo(buffer, lng, server_port, server_ip);
	status = ((count==lng) ? TRUE : FALSE);
	return status;
}
...
{
	size_t count = SendTo(ACK, strlen(ACK), remote_port, remote_ip);
...
{
	size_t count = SendTo(NAK, strlen(NAK), remote_port, remote_ip);
...
void ComChannel::test2(void)
{
	...
	e1.setDate(CalendarDate(2001,9,24));
	e1.setTime(TimeBlock(12,30,0));
	...
	e2.setDate(CalendarDate(2001,9,24));
	e2.setTime(TimeBlock(12,30,0));
