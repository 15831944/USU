	...
	if (!request_in_progress)
	{
		...
		if (status)
		{
			...
			switch (request_type)
			{
				...
				app->processQuery(client_port, client_ip, start_date, end_date);
			...
			case 'A':
			{
				...
				DialogWithLogging::Log(tmp);
				app->processNewEvent(client_port, client_ip);
				break;
			}
			default:
			{
...
BOOL ServerComChannel::sendCount(short count, const DString &remote_ip, UINT remote_port)
{
	...
	int tmp= SendTo(data, strlen(data), remote_port, remote_ip);
