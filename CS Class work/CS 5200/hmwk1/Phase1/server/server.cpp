...
// CServerApp::stop()
//
// This method stops the communications for the server 
//
...
{
	DString conf_file("C:\\Source Code\\CS Class work\\CS 5200\\hmwk1\\Phase1\\server.ini");
...
//	
void CServerApp::processQuery(UINT &clientPort, DString &clientIP, CTime &start_date, CTime &end_date)
{
	...
	for (int i=0; i<=max_index; i++)
	{
	    ...
	    {
		    Event* tmp = (Event*) event_set[i];
		    if (tmp->getDateTime() >= start_date && tmp->getDateTime() <= end_date)
			    eCount++;
	    }
	}

	if (!getComChannel()->sendCount(eCount,clientIP,clientPort))
	{
		t.Format("sendCount failed.\r\n");
		DialogWithLogging::Log(t);
		return;
	}
	else
	{
		t.Format("sendCount(%i) message successful.\r\n",eCount);
		DialogWithLogging::Log(t);
	}

	for (i=0; i<=max_index && eCount; i++)
	{
		if (event_set[i])
	    {
		    Event* tmp = (Event*) event_set[i];
			...
			{
				eCount--;
				getComChannel()->SendEvent(*tmp,clientIP,clientPort);
				result = getComChannel()->ReceiveAckOrNak(clientIP,clientPort,3000);
				if (result == 0)
				{
					t.Format("Client failed to acknowledge event.\r\n");
					DialogWithLogging::Log(t);
					return; // client not responding...
				}
				else
				{
					t.Format("Event acknowledged by client.\r\n");
					DialogWithLogging::Log(t);
				}
			}
	    }
...
//	
void CServerApp::processNewEvent(UINT &clientPort, DString &clientIP)
{
	// CS5700 TO DO: IMPLEMENT THIS METHOD
	int a = 0;
	getComChannel()->sendId(getNextEventId(),clientIP,clientPort);
	// since all data has already been entered the data should be received very quickly...
	Event * event = new Event;
	UINT remote_port;
	DString remote_ip;
	getComChannel()->ReceiveEvent(*event,remote_ip,remote_port,3000);
	addEvent(event);
	CString tmp;
	tmp.Format("Event added, acknowledgement sent.\r\n");
	DialogWithLogging::Log(tmp);
	getComChannel()->SendAck(remote_ip,remote_port);
}

UINT CServerApp::GetLocalPort()
{
	return local_port;
}
