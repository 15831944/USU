...
//	
void CServerApp::processQuery(const DString& client_ip, UINT client_port, CTime &start_date, CTime &end_date)
{
...
//	
void CServerApp::processNewEvent(const DString& client_ip, UINT client_port)
{
	DString tmp;
	DString tmp_client_ip(client_ip);
	UINT	tmp_client_port = client_port;

	long	new_id = getNextEventId();
	BOOL	status=FALSE;

	// Only try once at most.  If the ID message didn't through and then
	// we wait 5 minutes, the client will have already done else.
	if (com_channel->sendId(new_id, client_ip, client_port))
	{
		Event*	e = new Event;
		status = com_channel->ReceiveEvent(*e, tmp_client_ip, tmp_client_port, 300000);
		if (status)
		{
	        addEvent(e);
			tmp = "Received new event and added to database\r\n";
			DialogWithLogging::Log(tmp);		

			status = com_channel->SendAck(tmp_client_ip, tmp_client_port);
			if (status)
			{
				tmp = "ACK sent back to client\r\n";
				DialogWithLogging::Log(tmp);		
			}
		}
		else
		{
			tmp = "Error while waiting to receive event from client\r\n";
			DialogWithLogging::Log(tmp);
		}
	}
	else
	{
		tmp = "Error in sending new ID to the client\r\n";
		DialogWithLogging::Log(tmp);
	}
}
