...
Public dbConnection2 As rdoConnection
...
Sub Main()
    ...
    If InterfaceType = NASA Then
        If SelectMainViewFrm.OptionSelected = 3 Then
            Unload MDIMain
            End
        Else
            MDIMain.Show
            If SelectMainViewFrm.OptionSelected = 2 Then
                MDIMain.mnuViewRiskMeter_Click
            ElseIf SelectMainViewFrm.OptionSelected = 0 Then
                ' do setup
                RiskMonitorSetup.Show vbModal
            End If
        End If
    Else
        MDIMain.Show
...
Public Function DBConnect2(name As String) As Boolean
    ...
    ',"RiskMonitorSQLServer"
    Set dbConnection2 = dbEnvironment.OpenConnection(dsname:=name, _
        Prompt:=rdDriverNoPrompt, _
        Connect:="UID=;PWD=;", _
        Options:=rdAsyncEnable)
   
    While dbConnection2.StillConnecting
        DoEvents
    Wend
