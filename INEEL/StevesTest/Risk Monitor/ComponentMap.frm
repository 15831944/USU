...
Object = "{10A22AAA-AF8D-11D5-B022-0002E3059EB4}#3.0#0"; "DiagramControl.ocx"
Begin VB.Form ComponentMap 
   ...
   Begin SftTree.SftTree ComponentTree 
      ...
      ColWidth1       =   67
      ColTitle1       =   "Immediate Base Components"
      ColBmp1         =   "ComponentMap.frx":0442
      ColWidth2       =   67
      ColTitle2       =   "Total Base Components"
    ...
    If (Shift And 1) Or (Shift And 2) Then
        If FormMode = SelectOnly Then
            ...
            If RegionInfo.CompID > 0 Then
                ...
                If Index <> -1 Then
                    If RequestSelection(Index, False) Then
                        CompDiagram.SetSelection RegionIndex, True
                    End If
                Else
...
Private Sub DiagramControl1_LeftClickRegion(RegionInfo As DiagramControlProject.RegionType, RegionIndex As Integer, Shift As Integer)
...
Public Sub ClearSelectedNodes()
    ...
    CompDiagram.ClearSelections
    For i = SelectedComponentList.ListItems.Count To 1 Step -1
...
Private Sub ShowFormView()
    ...
    If FormView = DiagramView Then
        If NodeData(ActiveListIndex).hLevel = 0 Then
            ' try and load the top, initial file
            If CompDiagram.InitialDiagramFileName <> "" Then
                CompDiagram.ShowDiagram = True
                CompDiagram.DiagramFileName = CompDiagram.InitialDiagramFileName
            Else
                CompDiagram.ShowDiagram = False
            End If
        Else
            Dim pID As Integer
            pID = ComponentTree.Object.Parent(ActiveListIndex)
            If NodeData(pID).diagramPath <> "" And NodeData(pID).diagramPath <> "(none)" Then
                CompDiagram.DiagramFileName = NodeData(pID).diagramPath
                CompDiagram.ShowDiagram = True
            Else
                CompDiagram.ShowDiagram = False
            End If
...
Public Sub Form_Load()
    ...
    Load menuItems(6)
    menuItems(6).Caption = "Properties..."
    Load menuItems(7)
    menuItems(7).Caption = "View Movie"
    Load menuItems(8)
    menuItems(8).Caption = "View System Manuals"
...
Private Sub Form_Resize()
    If Me.Width > 3200 And ScaleHeight > 3000 Then
        ...
        SelectedComponentList.Height = ScaleHeight - 2835
        RemoveComponentButton.Top = ScaleHeight - 2370
...
Private Sub menuItems_Click(Index As Integer)
    ...
    Case 6 ' Properties...
        DoProperties
    Case 7 'View Movies
        ActiveMovieFile = pidDirectory & Left(NodeData(ActiveListIndex).diagramPath, Len(NodeData(ActiveListIndex).diagramPath) - 3) & "avi"
        ActivemovieForm.Visible = True
    Case 8 'View System Manuals
        BrowserManualFile = ""
        BrowserManualFile = pidDirectory & Left(NodeData(ActiveListIndex).diagramPath, Len(NodeData(ActiveListIndex).diagramPath) - 3) & "prl"
        If BrowserManualFile <> "" Then
            frmBrowser.StartingAddress = BrowserManualFile
            Browse.Visible = True
        End If
    End Select
...
Private Sub DeleteNode()
    ...
    If doDelete Then
        ...
        If (cBC > 0) Then
            Dim pL As Integer, pI As Integer
            pI = ActiveListIndex
            For pL = NodeData(ActiveListIndex).hLevel - 1 To 0 Step -1
                pI = ComponentTree.Object.Parent(pI)
                NodeData(pI).totalBaseComponents = NodeData(pI).totalBaseComponents - cBC
                ComponentTree.CellText(pI, 2) = NodeData(pI).totalBaseComponents
            Next pL
        End If
...
Private Sub SaveTree()
    If (changesMade) Then
        Dim i As Integer, pID As Integer, pI As Integer, j As Integer
        ...
        For i = 0 To ComponentTree.listCount - 1
            ...
            If NodeData(i).nodetype <> emptyNode Then
                ...
                pID = ComponentTree.Object.Parent(i)
                If pID >= 0 Then
                    pID = NodeData(pID).ID
                Else
                    pID = 0
                ...
                If NodeData(i).BaseComponentCount > 0 Then
                    ...
                    Do
                        ...
                        For j = 1 To NodeData(i).BaseComponentCount
                            If pI = i Then
                                mySQLstatement = "INSERT INTO [CompBaseComponent] (ComponentID, BaseCompID, Immediate) VALUES (" & NodeData(pI).ID & "," & NodeData(i).GetBaseComponent(j).ID & ",Yes)"
                            Else
                                mySQLstatement = "INSERT INTO [CompBaseComponent] (ComponentID, BaseCompID, Immediate) VALUES (" & NodeData(pI).ID & "," & NodeData(i).GetBaseComponent(j).ID & ",No)"
                            End If
                            dbConnection.Execute (mySQLstatement)
                        Next j
                        pI = ComponentTree.Object.Parent(pI)
